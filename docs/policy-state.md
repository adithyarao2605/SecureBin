...
- A sender-chosen release window starts at the first successful release; once `window_ends_at` passes, new authorizations take the uniform unavailable path while the original request token keeps its five-minute retry lease.
...
The Day 2 concurrency matrix and every policy added through Day 5 are **verified** by the pgTAP suite (9 files, 145 tests) and integration tests:
